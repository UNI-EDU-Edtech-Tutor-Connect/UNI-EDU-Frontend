...
import { BookOpen, MapPin, Calendar, Search, Filter, Eye, FileText, CheckCircle2, GraduationCap } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
              ...
              <DialogContent className="sm:max-w-md">
                ...
                <div className="flex gap-2 w-full mt-2">
                  <Button className="w-full bg-accent hover:bg-accent/90" asChild>
                    <Link href={`/dashboard/teacher/classes/${classItem.id}/test`}>Test nhận lớp ngay (Ưu tiên)</Link>
                  </Button>
                </div>
              </DialogContent>
            </Dialog>
            <Button className="flex-1 bg-accent hover:bg-accent/90" asChild>
              <Link href={`/dashboard/teacher/classes/${classItem.id}/test`}>
        ...
        <div>
          <div className="flex items-center gap-3">
            ...
            <Badge variant="default" className="bg-amber-500 hover:bg-amber-600">
              ...
              Đã kiểm định Bằng Đại học
            </Badge>
          </div>
          <p className="text-muted-foreground mt-1">Quản lý lớp học và tìm lớp dành riêng có ưu tiên xếp lớp.</p>
        </div>
      </div>
